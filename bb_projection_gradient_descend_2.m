function [min_result_at_x,min_result] = bb_projection_gradient_descend_2(x0, A, b, mu, opts6)
n = size(A,2);
...
g = zeros(2*n,1);
g1 = A'*(A*[eye(n),-1*eye(n)]*x-b)+mu*eye(n);
g2 = -A'*(A*[eye(n),-1*eye(n)]*x-b)+mu*eye(n);
g =[g1;g2];
 end
